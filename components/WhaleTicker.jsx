...
/**
 * Alt kayar bant.
 * - enabled=false => WS bağlanmaz, sadece staticText akar.
 * - enabled=true  => büyük işlemler/likidasyonlardan kısa özet üretir.
 */
export default function WhaleTicker({
  ...
  maxKeep = 30,
  enabled = true,
  staticText = "KriptoGözü • canlı panel"
}) {
  ...
  useEffect(()=> {
    if (!enabled) return; // kapalıysa hiç WS açma
  ...
  const line = useMemo(()=>{
    ...
    return queue.map(it => {
      ...
      return it.kind==="liq"
        ? `⚠️ Likidasyon • ${it.sym} • ${it.side} • ${usd}$`
        : `🐋 Büyük İşlem • ${it.sym} • ${it.side} • ${usd}$`;
    }).join("   •   ");
