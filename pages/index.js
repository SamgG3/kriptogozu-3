...
export default function CoinPage(){
  // ---- AYARLAR ----
  const symbol = 'BTCUSDT'          // İstersen sabit tut; ya da senin var olan state’ine bağla
  const REFRESH_MS = 3000           // 3 sn yenileme (canlıda WS’e geçilecek)

  // ---- STATE ----
  const [ohlc, setOhlc] = useState([])          // {open,high,low,close,time}
  const [price, setPrice] = useState(0)
  const [decimals, setDecimals] = useState(2)
  const [signals, setSignals] = useState([])    // Bildirim listesi (AI sinyalleri)
  ...
  useEffect(()=>{
    async function load(){
      ...
      setDecimals(data.priceDecimals ?? 2)  // tickSize’dan türetilebiliyorsa backend bunu dönmeli
    }
    load()
  ...
  // ---- AI SİNYAL MOTORU → BİLDİRİMLER ----
  useEffect(()=>{
    const id = setInterval(()=>{
      if (!ohlc.length) return
      const s = generateSignalFromOHLC(ohlc)  // {side, confidence, reason, time}
  ...
  return (
    <div className="max-w-6xl mx-auto p-4 md:grid md:grid-cols-[2fr,1fr] md:gap-6">
      ...
      <section className="space-y-6">
        ...
        {/* Trend kırılım rozeti ve uyarı notu */}
