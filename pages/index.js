...
function biasFromLatest(L) {
  ...
  return { longPct, shortPct: 100-longPct, score };
}
...
export default function Home() {
  ...
  useEffect(()=>{ if(typeof window!=="undefined"){ try{ localStorage.setItem("kgz_favs", JSON.stringify(favs)); }catch{} } },[favs]);
  const isFav = (sym)=> favs.includes(sym);
  const toggleFav = (sym)=> setFavs(prev=> isFav(sym) ? prev.filter(s=>s!==sym) : [...prev, sym]);
      ...
      <main style={{ padding:"16px 18px" }}>
        ...
        <div style={{ display:"flex", gap:12, alignItems:"center", marginBottom:12, flexWrap:"wrap" }}>
          ...
          <div style={{ display:"flex", gap:8, alignItems:"center", marginLeft:"auto" }}>
            <input
              value={q} onChange={(e)=>setQ(e.target.value)} placeholder="BTC, ETH, SOL…"
        ...
        <div style={{ border:"1px solid #23283b", borderRadius:10, overflow:"hidden", marginBottom:14, background:"#121625" }}>
          <div style={{
            ...
            padding:"10px 12px", background:"#0e1424", color:"#a9b4c9", fontWeight:700
          }}>
            <div>• WebSocket: Canlı</div><div>Fiyat</div><div>Long/Short</div><div>24s Değişim</div><div>Risk</div>
          </div>
          ...
          {symbols.map((sym)=>{
            ...
            return (
              <div key={sym} style={{ display:"grid", gridTemplateColumns:"1.2fr 1fr 1fr 1fr 0.8fr",
                ...
                <div>
                  <span style={{ padding:"4px 8px", borderRadius:20, border:"1px solid #2a2f45", background:"#151a2b", color:risk.color, fontWeight:800 }}>
                    {risk.txt}
                  </span>
                </div>
              </div>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit, minmax(300px, 1fr))", gap:12 }}>
          {symbols.map((sym)=> <CoinCard key={sym} sym={sym} row={rows[sym]} ws={wsTicks[sym]} fav={isFav(sym)} onFav={()=>toggleFav(sym)} />)}
        </div>
...
/* ==================== Kart ==================== */
function CoinCard({ sym, row, ws, fav, onFav }) {
  const L = row?.latest || {};
